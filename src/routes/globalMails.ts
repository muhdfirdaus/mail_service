...
import { cache, GLOBAL_MAILS_CACHE_PREFIX, makeETag, bustGlobalMailsCache } from "../lib/cache";
...
r.post("/", async (req, res) => {
  ...
  const {
    legacy_mail_id,
    status,                 // <— capture and REMOVE from $set
    create_time: _ct,
    delete_time: _dt,
    update_time: _ut,
    ...rest
  ...
  const doc = await GlobalMail.findOneAndUpdate(
    { legacy_mail_id },
...
r.post("/:legacy_mail_id/_patch", async (req, res) => {
  ...
  const doc = await GlobalMail.findOneAndUpdate(
    { legacy_mail_id, delete_time: NOT_DELETED_DATE },
...
r.delete("/:legacy_mail_id", async (req, res) => {
  ...
  const doc = await GlobalMail.findOneAndUpdate(
    { legacy_mail_id },
    { $set: { delete_time: new Date(), update_time: new Date(), status: 0 } },
    { new: true }
  );
  res.json(doc);
});
...
r.post("/:legacy_mail_id/releases", async (req, res) => {
  ...
  const gm = await GlobalMail.findOne({ legacy_mail_id, delete_time: NOT_DELETED_DATE });
  if (!gm) return res.status(404).json({ error: "global_mail not found" });

  // Enforce single release per mail
  const existing = await MailRelease.findOne({ legacy_mail_id, delete_time: NOT_DELETED_DATE });
  if (existing) return res.status(409).json({ error: "release already exists for this mail" });
  ...
  if (!release_legacy_id) {
    release_legacy_id = parseInt(crypto.randomBytes(3).toString("hex"), 16);
  }
  ...
  const rel = await MailRelease.findOneAndUpdate(
    filter,
...
r.post("/:legacy_mail_id/releases/:release_legacy_id/cancel", async (req, res) => {
  ...
  await MailRelease.updateOne(
    { legacy_mail_id: Number(legacy_mail_id), release_legacy_id: Number(release_legacy_id) },
    { $set: { state: "cancelled", update_time: new Date() } }
  );
  res.json({ ok: true });
});
...
r.post("/:legacy_mail_id/releases/:release_legacy_id/recall", async (req, res) => {
  const { legacy_mail_id, release_legacy_id } = req.params;
  const now = new Date();
  const result = await PlayerMail.updateMany(
    { legacy_mail_id: Number(legacy_mail_id), release_legacy_id: Number(release_legacy_id), is_claimed: false },
...
r.post("/:legacy_mail_id/cancel-release", async (req, res) => {
  const legacy_mail_id = Number(req.params.legacy_mail_id);
  const now = new Date();
  const doc = await MailRelease.findOneAndUpdate(
    { legacy_mail_id, delete_time: NOT_DELETED_DATE },
    { $set: { state: "cancelled", status: 0, update_time: now } },
    { new: true }
  );
  if (!doc) return res.status(404).json({ error: "release not found" });
  res.json({ ok: true });
});
...
r.post("/:legacy_mail_id/recall", async (req, res) => {
  ...
  const now = new Date();
  const result = await PlayerMail.updateMany(
    { legacy_mail_id, is_claimed: false, delete_time: NOT_DELETED_DATE },
    { $set: { status: 0, delete_time: now, update_time: now } }
  );
  res.json({ ok: true, recalled: result.modifiedCount });
});
...
r.get("/", async (req, res) => {
  const scope = String(req.query.scope || "active").toLowerCase() as
    | "active" | "upcoming" | "expired" | "all";
  const page = Math.max(1, Number(req.query.page || 1));
  const pageSize = Math.min(100, Math.max(1, Number(req.query.pageSize || 20)));
  const now = new Date();

  const key = `${GLOBAL_MAILS_CACHE_PREFIX}${scope}:${page}:${pageSize}`;
  ...
  const cached = cache.get<{ payload: any; etag?: string }>(key);
  if (cached) {
  ...
  const base: any = { delete_time: NOT_DELETED_DATE, status: 1 };
  ...
  const docs = await GlobalMail.find(filter).lean();
  ...
  docs.sort((a: any, b: any) => {
    ...
    if (aStart !== bStart) return bStart - aStart; // newest start_at first
  ...
  const mapped = itemsForPage.map((gm: any) => ({
    ...
    is_active:
      gm.schedule?.start_at &&
      new Date(gm.schedule.start_at) <= now &&
      (gm.schedule?.end_at == null || new Date(gm.schedule.end_at) > now),
  }));

  const maxUpdated = itemsForPage.reduce<number>(
    (mx, it: any) => Math.max(mx, it.update_time ? new Date(it.update_time).getTime() : 0),
    0
  );
  ...
  const etag =
    itemsForPage.length > 0
      ? makeETag([
          ...
          page,
          pageSize,
          ...itemsForPage.map((x: any) => x.legacy_mail_id),
          maxUpdated,
        ])
      : undefined;
  ...
  cache.set(key, { payload, etag }, 30_000, etag);
